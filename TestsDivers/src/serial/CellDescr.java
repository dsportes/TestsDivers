...
public class CellDescr {
	...
	public Cell.CellNode newCellNode(Cell cell, String nodeType) throws Exception {
		if (nodeType == null || cell == null)
			throw new Exception("cellNodeFactory() invoked without nodeType or cell parameter");
		NodeDescr nd = getNodeDescr(nodeType);
		if (nd == null)
			throw new Exception("newCellNode() invoked with unknown nodeType ["
					+ nodeType + "] cell class :[" + cell.getClass().getName() + "]");
		Cell.CellNode cn = nd.newCellNode(cell);
		if (cn == null)
			throw new Exception("newCellNode() invoked on [" + nodeType
	 ...
	 * @throws AppException
	 */
	public void read(Cell target, byte[] bytes) throws Exception {
		...
		try {
			...
			while (true) {
				...
				read(in, cn, nd);
				if (cn != null) {
					if (cn.version() > target.version())
						target.version = cn.version();
			...
			throw new Exception("CellDescr:read - " + this.getClass().getSimpleName(), e);
		}

	}

	public ArrayList<IW> read(Object owner, byte[] bytes) throws Exception {
	 ...
	 * @throws AppException
	 */
	public byte[] serialize(Cell target) throws Exception {
		...
		try {
			...
			throw new Exception("CellDescr:serialize - " + this.getClass().getSimpleName(), e);
		}
	}

	public byte[] serialize(Collection<IW> a) throws Exception {
	...
	public void serial(Map<String,?> json, Serial serial) throws Exception {
		...
		for (Object obj1 : entries) {
			@SuppressWarnings("unchecked")
			Entry<String,?> entry = (Entry<String,?>)obj1;
			...
			if (nd.singleKey != null) {
				...
				try {
					...
					for (Object obj2 : nodes) {
						@SuppressWarnings("unchecked")
						Map<String,?> node = (Map<String,?>) obj2;
						v = 0;
						if (node != null && (v = nd.serialNode(out, node)) != 0) {
							if (v > serial.version) serial.version = v;
							hasNodes = true;
	...
	public StringBuffer toJSON(StringBuffer sb, Cell target, String[] filterKeys,
		...
		for (String filter : filterKeys) {
			...
			for (CellNode cnx : nodes) {
				...
				String s = nd.toJSON(cnx, isAdmin, cnx.jsonFields(filterArg));
	...
	private class NodeDescr {
		...
		private long serialNode(DataOutputStream out, Map<String,?> node) throws IOException {
			if (node == null) return 0;
			boolean first = true;
			Long version = null;
			try { version = (Long) node.get("version"); } catch (Exception e){	}
			if (version == null || version == 0) return 0;
		...
		private class FieldDescr {
			...
			private void writeObj(DataOutputStream out, Object value) {
				try {
					...
					case 8:
					case 9: { // Array Long / Integer / Short
						List<?> a = (List<?>)value;
